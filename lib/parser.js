...
Parser.prototype.parseFile = function(filename, encoding) {
    ...
    return self.parseBinaryString(fileContent, encoding);
};

/**
 * Execute Fileparsing
 */
Parser.prototype.parseBinaryString = function(binaryString, encoding) {
    var self = this;
    var filename = self.filename;
    if (!self.filename) {
      filename = self.filename = self.extension = "binaryString";
    }

    if (typeof(encoding) === 'undefined')
        encoding = 'utf8';
    iconv.skipDecodeWarning = true;
    self.src = iconv.decode(binaryString, encoding);
    ...
    self.elements = self.blocks.map(function(block, i) {
        var elements = self.findElements(block, self.filename);
